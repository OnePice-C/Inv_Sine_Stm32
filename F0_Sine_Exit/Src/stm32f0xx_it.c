...
void TIM1_BRK_UP_TRG_COM_IRQHandler(void) {
	if (TIM_GetITStatus(TIM1, TIM_IT_Update) != RESET) { //if update flag turns on
		...
		if (i == 110) {
			...
			Read_ADC_Pic = RegularConvData_Tab[1];	//read adc
		}
		/*		if (chonmang == 1) {

		 TIM_SetCompare2(TIM1, lookUp4[i]);
		 TIM_SetCompare1(TIM1, lookUp4[i1]);

		 } else {

		 TIM_SetCompare2(TIM1, lookUp3[i]);
		 TIM_SetCompare1(TIM1, lookUp3[i1]);
		 }*/
		if (i >= 434 ) {
			i = 0;
		}
