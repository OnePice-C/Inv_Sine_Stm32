...
extern volatile int16_t Read_ADC_Pic;
extern volatile uint16_t i;
extern volatile float Ext_K;
...
int main(void) {
	...
	while (1) {
		...
		if (RegularConvData_Tab[0] >= 806 && RegularConvData_Tab[0] <= 1000) {
			for (int8_t num = 0; num < 10; num++) {
			...
			if (RegularConvData_Tab[0] >= 806
					&& RegularConvData_Tab[0] <= 1000) {
				GPIOA->BSRR |= (1 << 15);
				NVIC_DisableIRQ(TIM1_BRK_UP_TRG_COM_IRQn);
				TIM_CtrlPWMOutputs(TIM1, DISABLE);
			}
...
static void TIM_Config(void) {
	...
	TIM_BDTRStructure.TIM_DeadTime = 22;
